...
#define refresh_rate        500000     'sampling frequency of sweep process. (MCU_frequency / sweep_processdelay)
#define MCU_frequency       300E6     'Processor frequency
#define sweep_processdelay  600      'must be high enough to don't overload ADwin.
#define DAC_ZERO            32768
#define output_card         3
#define input_card          2
#define input_channel       2
#define nb_outs             8         'number of outputs
#define fifo_len_dc_ro      1000003
...
dim start1, start2, start3, start4, start5, start6, start7, start8, i, sweep_cycle, input as long
dim inc1, inc2, inc3, inc4, inc5, inc6, inc7, inc8 as float
dim sweep_start[nb_outs], sweep_stop[nb_outs] as long at dm_local 

dim fifo_dc_readout[fifo_len_dc_ro] as long as fifo 
...
init:
  ...
  sweep_cycle = 0
  sweep_active = 2 'intermediate state to enable flag directly at sweep start.
  
  P2_ADCF_Mode(2, 1)
  
event:
  ' READ INPUT CARD AT BEGINNING OF EVENT CYCLE
  input = P2_Read_ADCF24(input_card, input_channel)
  ...
  if (sweep_active = 1) then
    ...
    ' WRITE MEASURED VALUE DIRECTLY TO FIFO
    fifo_dc_readout = Shift_Right(input, 6)
