...
dim nb_lockin_cycles, lockin_cycle, lockin_out, lockin_in as long
dim kappa, c0, s0, c1, c2, c3, c4, s1, s2, s3, s4 as float                     'lockin modulation parameters????? int working if everything is in integers?

dim fifo_inphase[fifo_len], fifo_quadrature[fifo_len] as long as fifo   'send data to PC
...
init:
  ...
  'SET FILTER KONSTANT
  kappa = 1-exp(-(process_delay * cycle_time)/ tao)
...
event:
  if (lockin_active = 1) then
    ...
    lockin_in = Shift_Right(lockin_in, 6)
    
    'WRITE LOCKIN OUTPUT (ASAP AFTER READING INPUT)
    P2_DAC(lockin_card, lockin_channel, lockin_bias + lockin_sin[lockin_cycle])
  
    'LOCK-IN CALCULATION AND FLITER (157 CYCLES)
    c0 = (lockin_in - DAC_ZERO_18) * 2 * (lockin_cos[i - 1])   'phase i-1 of the previous output
    s0 = -(lockin_in - DAC_ZERO_18) * 2 * (lockin_sin[i - 1])   'phase i-1 of the previous output
    '4 x 1st order butterworth filter after another -> 4th order in total
    c1 = c1 + kappa * (c0-c1)
    s1 = s1 + kappa * (s0-s1)
    c2 = c2 + kappa * (c1-c2)
    s2 = s2 + kappa * (s1-s2)
    ...
    if (lockin_cycle = nb_lockin_cycles) then
      ' SEND DATA TO PC (SUBSAMPLED TO LOCKIN FREQUENCY)
