...
class Windfreaktech_SynthHD(Instrument):
	...
	def reload_calibration(self):
		...
		'''
		try:
			data = np.loadtxt(qt.config.get('user_insdir')+'/Windfreaktech_SynthHD.cal')
			f = data[1:,0]
			p = data[0,1:]
			values = data[1:,1:]
	...
	def do_set_power(self,power,channel):
		...
		if(np.isnan(float(self._interp_amplitude((self._frequency[channel],power))))):
			raise ValueError('The possible minimum and maximum output power of this microwave source does depend on the frequency. Unfortunately, your desired combination of frequency and output power is not possible.')
		else:
