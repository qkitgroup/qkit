...
# IV samples and rates
samples = 100
rate = 5
# number of sweeps per [mag|mw] scan
sweeps =  1
...
def gen_triangle(min,max,samples=1000):
    tri_fw = linspace(min,max,samples)
    tri_bw = linspace(max,min,samples)
    return tri_fw,tri_bw

def take_IV(vec_fw,vec_bw,vec_2,out_conversion_factor,in_amplification,out_divider=1, in_offset=0):
