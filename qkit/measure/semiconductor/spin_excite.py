...
from qkit.measure.semiconductor.manipulation_backends.MA_backend_base import (
    MA_backend_base,
)
...
def expand_mapping(dictionary, mapping):
    additional_mapping = {
        entry: entry for entry in dictionary.keys() if entry not in mapping.keys()
    }
...
class Qupulse_decoder2:
    ...
    def _validate_entries(self):
        ...
        for pt, pars in self.experiments:
            ...
            if type(pt) not in self.valid_pulses:
                raise TypeError(
                    f"{__name__}: Cannot use {pt} as pulse template. Must be a qupulse pulse template."
                )
            if not pt.identifier:
                warnings.warn(
                    f"{__name__}: Pulse template {pt} has no identifier. Support messages will be less clear."
                )
            if type(pars) != dict:
                raise TypeError(
                    f"{__name__}: Cannot use {pars} as pulse template parameters. Must be a dictionary."
                )
            ...
            for channel in pt.defined_channels:
                if channel in pt_channels:
                    raise ValueError(
                        f"{__name__}: Channels of different experiments in {pt.identifier} overlap. Experiments are not allowed to share channels."
                    )
                pt_channels.add(channel)
            for measurement in pt.measurement_names:
                if measurement in pt_measurements:
                    raise ValueError(
                        f"{__name__}: Measurements of different experiments in {pt.identifier} overlap. Experiments are not allowed to share Measurements."
                    )
            ...
            if isinstance(pt, self._for_type):
                ...
                if isinstance(a, str) and a in pt_axis:
                    raise ValueError(
                        f"{__name__}: The step parameter defined in {pt.identifier} is already used in another experiment. Experiments must have different step parameter names."
                    )
                pt_axis.add(a)
    ...
    def _validate_channel_sample_rates(self, channel_sample_rates):
        ...
        if missing_rates:
            raise ValueError(
                f"{__name__}: Incomplete instructions by {channel_sample_rates}. The following channels have no assigned sampling rates:\n{missing_rates}"
            )
    ...
    def _validate_measurement_sample_rates(self, measurement_sample_rates):
        ...
        if missing_rates:
            raise ValueError(
                f"{__name__}: Incomplete instructions by {measurement_sample_rates}. The following channels have no assigned sampling rates:\n{missing_rates}"
            )
    ...
    def _render_channel(self, wvf, channel, sample_rate):
        ...
        if not round(float(sample_count), 10).is_integer():
            warnings.warn(
                f"{__name__}: Sample count {sample_count} is not an integer. Will be rounded (this changes the sample rate)."
            )
        times = np.linspace(
            float(start_time), float(end_time), num=int(sample_count), dtype=float
        )
            ...
            for channel in wvf.defined_channels:
                ...
                if deep_render:
                    if type(wvf) == self._repetition_type:
                        samples = self._render_channel(
                            wvf._body, channel, self.channel_sample_rates[channel]
                        )
                        ...
                        for sub_wvf in wvf._sequenced_waveforms:
                            self.channel_pars[channel]["samples"].append(
                                self._render_channel(
                                    sub_wvf, channel, self.channel_sample_rates[channel]
                                )
                            )
                    else:
                        raise TypeError(
                            f"{__name__}:  Deep rendering failed. {wvf} does not contain any sub-waveforms"
                        )
                else:
                    self.channel_pars[channel]["samples"].append(
                        self._render_channel(
                            wvf, channel, self.channel_sample_rates[channel]
                        )
                    )
    ...
    def _extract_measurement_pars(self):
            ...
            for measurement, parameters in prog.get_measurement_windows().items():
                ...
                if (
                    measurement_durations[
                        measurement_durations != measurement_durations[0]
                    ].size
                    > 0
                ):  # check whether all elements are the same
                    different_window_lengths += f"{measurement}\n"
                self.measurement_pars[measurement] = {}
                self.measurement_pars[measurement]["measurement_count"] = len(
                    measurement_durations
                )
                self.measurement_pars[measurement]["measurement_duration"] = (
                    measurement_durations[0] * 1e-9
                )
                self.measurement_pars[measurement]["sample_count"] = np.int32(
                    np.floor(
                        self.measurement_pars[measurement]["measurement_duration"]
        ...
        if different_window_lengths:
            raise ValueError(
                f"{__name__}: All measurement windows for one measurement have to be of the same length. \
    ...
    def _extract_axis_pars(self):
        ...
        for pt, pars in self.experiments:
            if isinstance(pt, self._for_type):
                ...
                if not pt.measurement_names:
                    warnings.warn(
                        f"{__name__}: {pt.identifier} does not contain any measurements. Pulse parameter axis cannot be extracted automatically."
                    )
                for measurement in pt.measurement_names:
                    self.measurement_pars[measurement][
                        "loop_step_name_pp"
                    ] = loop_step_name
                    self.measurement_pars[measurement]["loop_range_pp"] = (
                        np.arange(loop_start, loop_stop, loop_step) * 1e-9
                    )
            else:
                warnings.warn(
                    f"{__name__}: {pt.identifier} is not a ForLoopPulseTemplate. Pulse parameter axis cannot be extracted automatically."
                )
        ...
        for measurement, settings in self.measurement_pars.items():
            ...
            settings["loop_range_tt"] = np.linspace(
                0, t_sample * settings["sample_count"], settings["sample_count"]
            )
...
class Settings:
    ...
    def _assert_measurement_averages(self, measurement_params, averages):
        ...
        if missing_averages:
            raise ValueError(
                f"{__name__}: Incomplete instructions by {averages}. The following measurements have no assigned averages:\n{missing_averages}"
            )
    ...
    def _validate_channel_entries(self):
        ...
        if unsupported_channels:
            raise AttributeError(
                f"{__name__}: Your manipulation backend does not support the following channels:\n{unsupported_channels}"
            )
    ...
    def _validate_measurement_entries(self):
        ...
        if unsupported_measurements:
            raise AttributeError(
                f"{__name__}: Your readout backend does not support the following measurements:\n{unsupported_measurements}"
            )

    def _get_measurement_units(self):
        for measurement, settings in self.measurement_settings.items():
            settings["unit"] = self.core._ro_backend._registered_measurements[
                measurement
            ]["unit"]

    def _get_measurement_nodes(self):
        for measurement, settings in self.measurement_settings.items():
            settings["data_nodes"] = self.core._ro_backend._registered_measurements[
                measurement
            ]["data_nodes"]
    ...
    def load(self):
        ...
        for measurement, settings in self.measurement_settings.items():
            getattr(self.core._ro_backend, f"{measurement}_set_measurement_count")(
                settings["measurement_count"]
            )
            getattr(self.core._ro_backend, f"{measurement}_set_sample_count")(
                settings["sample_count"]
            )
            getattr(self.core._ro_backend, f"{measurement}_set_averages")(
                settings["averages"]
            )
...
class Exciting(mb.MeasureBase):
    ...
    def report_static_voltages(self, yesno):
        if not isinstance(yesno, bool):
            raise TypeError(
                f"{__name__}: Cannot use {yesno} as report_static_voltages. Must be a boolean value."
            )
    ...
    def mode(self, new_mode):
        if new_mode not in self.modes:
            raise AttributeError(
                f"{__name__}: {new_mode} is not a valid measurement mode. Allowed modes are: \n {self.modes}"
            )
        self._mode = new_mode
    ...
    def _validate_RO_backend(self, RO_backend):
        if not issubclass(RO_backend.__class__, RO_backend_base):
            raise TypeError(
                f"{__name__}: Cannot set {RO_backend} as readout backend. The backend must be a subclass of RO_backend_base"
            )

    def _validate_MA_backend(self, MA_backend):
        if not issubclass(MA_backend.__class__, MA_backend_base):
            raise TypeError(
                f"{__name__}: Cannot set {MA_backend} as manipulation backend. The backend must be a subclass of MA_backend_base"
            )

    def update_t_parameters(self, vec, coordname, measurement, unit="s"):
        new_t_parameter = self.Coordinate(
            coordname,
            unit=unit,
    ...
    def update_pulse_parameters(self, vec, coordname, measurement, unit=" a.u."):
        new_pulse_parameter = self.Coordinate(
            coordname,
            unit=unit,
            values=np.array(vec, dtype=float),
            set_function=lambda val: True,
    ...
    def _prepare_measurement_file(self, data, coords=()):
        ...
        if self.report_static_voltages:
            ...
            _instr_settings_dict = get_instrument_settings(
    ...
    def _prepare_measurement(self, coords):
        ...
        for measurement in self.settings.measurement_settings.keys():
            total_iterations += self.settings.measurement_settings[measurement][
                "averages"
            ]
            ...
            for node in self.settings.measurement_settings[measurement]["data_nodes"]:
                ...
                if self.mode == "timetrace":
                    datasets.append(
                        self.Data(
                            name="%s.%s" % (measurement, node),
                            coords=coords + [self._t_parameters[measurement]],
                            unit=self.settings.measurement_settings[measurement][
                                "unit"
                            ],
    ...
    def _check_node_data(self, latest_node_data):
        if latest_node_data.ndim != 3:
            raise IndexError(
                f"{__name__}: Invalid readout dimensions. {self._ro_backend} must return arrays with 3 dimensions."
            )
        if np.size(latest_node_data, axis=2) == 0:
            raise ValueError(
                f"{__name__}: The last call of {self._ro_backend}.read() returned an array with empty slices."
            )
    ...
    def _measure_vs_pulse_parameter(self, dimension, progress_bar):
        ...
        while not self._ro_backend.finished():
            ...
            for measurement in latest_data.keys():
                ...
                if len(latest_data[measurement][first_node]) == 0:
                    continue
                ...
                for node in latest_data[measurement].keys():
                    ...
                    if self.divider[measurement] == 1:
                        total_sum[measurement][node] = np.average(
                            latest_node_data, axis=(0, 2)
                        )
                        self._datasets["%s.%s" % (measurement, node)].append(
                            total_sum[measurement][node]
                        )
                    else:
                        total_sum[measurement][node] += np.average(
                            latest_node_data, axis=(0, 2)
                        ...
                        if dimension == 1:
                            self._datasets["%s.%s" % (measurement, node)].ds[:] = (
                                total_sum[measurement][node] / self.divider[measurement]
                            )
                        elif dimension == 2:
                            self._datasets["%s.%s" % (measurement, node)].ds[-1] = (
                                total_sum[measurement][node] / self.divider[measurement]
                            )
                        elif dimension == 3:
                            self._datasets["%s.%s" % (measurement, node)].ds[-1][-1] = (
                                total_sum[measurement][node] / self.divider[measurement]
                            )
                self.divider[measurement] += 1
    ...
    def _choose_measurement_function(self, dimension, progress_bar):
        if self.mode == "pulse_parameter":
            ...
            if dimension < 2:
                raise ValueError(
                    f"{__name__}: pp_vs_t mode requires at least a 2D measurement."
                )
            self._measure_pp_vs_time(dimension, progress_bar)
        else:
            raise AttributeError(
                f"{__name__}: {self.mode} is not a valid measurement mode. Allowed modes are: \n{self.modes}"
            )

    def _create_axis(self):
        for name, measurement in self.settings.measurement_settings.items():
            self.update_pulse_parameters(
                measurement["loop_range_pp"], measurement["loop_step_name_pp"], name
            )
            self.update_t_parameters(
                measurement["loop_range_tt"], measurement["loop_step_name_tt"], name
            )
    ...
    def compile_qupulse(
        ...
        **add_pars,
        ...
        self.mode = mode
        channel_sample_rates = {
            channel: getattr(self._ma_backend, f"{channel}_get_sample_rate")()
            for channel in self._ma_backend._registered_channels.keys()
        }
        measurement_sample_rates = {
            measurement: getattr(self._ro_backend, f"{measurement}_get_sample_rate")()
            for measurement in self._ro_backend._registered_measurements.keys()
        }

        decoded = Qupulse_decoder2(
            *experiments,
            channel_sample_rates=channel_sample_rates,
            measurement_sample_rates=measurement_sample_rates,
            deep_render=deep_render,
            **add_pars,
        )

        self.settings = Settings(
            self, decoded.channel_pars, decoded.measurement_pars, averages, **add_pars
        )
    ...
    def measure3D(self):
        ...
        pb = Progress_Bar(
            len(self._x_parameter.values)
            * len(self._y_parameter.values)
...
if __name__ == "__main__":
    ...
    from qupulse.pulses import ForLoopPT, PointPT

    # create our atomic "low-level" PointPTs
    first_point_pt = PointPT(
        [(0, "v_0"), (1, "v_1", "linear"), ("t", "v_0+v_1", "jump")],
    ...
    parameters = dict(t=3, t_2=2, v_0=1, v_1=1.4, t_start=4, t_end=13)

    import qkit
    from qkit.measure.semiconductor.manipulation_backends.MANIP_test_backend import (
        MA_test_backend,
    )
    ...
    excitation = Exciting(
        ro_backend,
        ma_backend,
        (for_loop_pt, parameters),
        averages={"Blurps": 100},
        channel_mapping={"patushka": "Ch1"},
