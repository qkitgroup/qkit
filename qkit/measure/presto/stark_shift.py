    ...
    def run(
        ...
        with pulsed.Pulsed(
            ...
            **CONVERTER_CONFIGURATION,
            ...
            scale = np.concatenate(([self.readout_amp],self.readout_stark_arr))
            ...
            for j in range(len(self.readout_stark_arr)):
                for _ in range(len(control_if_arr)):
                    # Control pulse
                    pls.select_scale(T,j+1, self.readout_port, group=0)
                    pls.reset_phase(T, self.readout_port)
                    pls.output_pulse(T, readout_pulse)
                    # Readout pulse starts right after control pulse
                    T += self.readout_duration-self.control_duration
                    pls.reset_phase(T, self.control_port)
                    pls.output_pulse(T, control_pulse)
                    T += self.control_duration
                    pls.select_scale(T,0, self.readout_port, group=0)
                    pls.reset_phase(T, self.readout_port)
                    pls.output_pulse(T, readout_pulse)
                    # Sampling window
