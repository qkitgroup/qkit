...
from qkit.measure.presto._base import Base,get_instr_dict
    ...
    def run(
        ...
        with pulsed.Pulsed(
            ...
            **CONVERTER_CONFIGURATION,
            ...
            ns_match = int(round(self.match_duration * pls.get_fs("adc")))
            templ_e = np.full(ns_match, self.pos_e) # (self.pos_e+self.pos_g)/(np.abs(self.pos_e+self.pos_g)))
            templ_g = np.full(ns_match,  self.pos_g)  #(self.pos_e-self.pos_g)/(np.abs(self.pos_e-self.pos_g)))
            print(ns_match)
            match_e_conditional, match_g_conditional = pls.setup_template_matching_pair(
                input_port=self.sample_port,
                template1 = -templ_e,
                template2 = templ_g,
                )
...
#             pls.store(T + self.readout_sample_delay)
            ...
            pls.match(T + self.readout_match_delay, [match_e_conditional, match_g_conditional])
            
            T += self.readout_duration+ self.readout_match_delay+0e-9
...
#             self.t_arr, self.store_arr = pls.get_store_data()
            ...
            self.match_arr_conditional = pls.get_template_matching_data([match_i, match_q])
