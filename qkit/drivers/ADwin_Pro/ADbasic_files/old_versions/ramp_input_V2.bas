...
Dim ramp_down_busy, i, channel, increment_sign, output_number, module_number, skips, skips_total, eval, initialized, delay_ADC, iterator_parallel, iterator_oversampled, step_oversampling as Long
...
LowInit:  
  ...
  'ADC cannot be read and averaged every event so it is delayed by the variable delay_ADC. =1 means it skipps every second event
  delay_ADC=1
...
Event: 
  ...
  SelectCase run_Par 'run variable'
        ...
        Case 0 'initialize rampvalue and ramping sign'
          ...
          If ((stopvalue <= upper_limit_Data[channel]) And (stopvalue >= lower_limit_Data[channel])) Then
            ...
            If (safe_bit_Data[channel]=1) Then 
              'channel is a safe port so 0.5V/s. so 20V/(2^16)/(2*10^-6s)/0.5V/s=305 skips between minimal voltage increments'
            ...
            Else
              ...
              'If skips = 10 then every 10th clock cycle the voltage will be increased by the minimum value. 2*10V/(2^16)/(2*10^-6s)= 152V/s'
              skips_total = Round(152 / ramping_speed_FPar * (voltage_range_Data[channel] / 10)) 'FPar_77 is speed in Volts/s'
            EndIf
    ...
    Case 2 'read inputs every 4us'
      'ADC cannot be read and averaged every event so it is delayed by the variable delay_ADC
      IF (delay_ADC=1) Then
      ...
      Inc(delay_ADC)
      IF (delay_ADC>2) Then '2 means it skipps every second event
        delay_ADC=1
      EndIF
    ...
    Case 3 'voltage ramps in parallel'
      For iterator_parallel=1 To 12    'iterate over 20 gates
            ...
            Case 0 'initialize rampvalue and ramping sign'
              ...
              If ((ramp_stop_Data[ramped_channels_parallel_Data[iterator_parallel]] <= upper_limit_Data[ramped_channels_parallel_Data[iterator_parallel]]) And (ramp_stop_Data[ramped_channels_parallel_Data[iterator_parallel]] >= lower_limit_Data[ramped_channels_parallel_Data[iterator_parallel]])) Then
                ...
                If (safe_bit_Data[ramped_channels_parallel_Data[iterator_parallel]]=1) Then 
                  'channel is a safe port so 0.5V/s. so 2*10V/(2^16)/(2*10^-6s)/0.5V/s=305 skips between minimal voltage increments'
                  'The ramping speed is not increased for safe ports of the voltage range is lowered by voltage dividers.
                  skips_total_parallel_Data[iterator_parallel] = 305
                Else
                  'channel is a normal port. The ramping speed (so amount of skips is adjusted also with the maximum voltage range of the outputs.'
                  'If skips = 10 then every 10th clock cycle the voltage will be increased by the minimum value. 2*10V/(2^16)/(2*10^-6s)= 152V/s'
                  skips_total_parallel_Data[iterator_parallel] = Round(152 / ramping_speed_FPar * (voltage_range_Data[ramped_channels_parallel_Data[iterator_parallel]] / 10)) 'FPar_77 is speed in Volts/s'
                EndIf
  ...
  '--------------------------------------------------------------------------------
  'Oversmapling continuously of a couple of gates every event at 500kHz
