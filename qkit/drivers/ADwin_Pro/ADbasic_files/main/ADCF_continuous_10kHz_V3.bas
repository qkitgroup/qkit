...
'<Header End>
'Process writes 10kHz ADC input 1 values each event (1kHz) to memory of ADwin. Each value is an average of 32 measurements.  
'One buffer cycle takes 0.96 seconds
...
#Define max_val 4*8*300 '1e5                                'must be devisible by 32 (=4*8) no. of values
...
LowInit:
  ...
  P2_Set_Average_Filter(pattern, 5)                        '2**5 averages
  P2_Burst_Init(module, 1, 0, max_val, 10000, 010b)        '100 000ns per sample, 10 kHz
  P2_Burst_Reset(pattern)
  P2_Burst_Start(pattern)
  
  
Event:
  Processdelay = 1e6 '1e6
  mem_idx = P2_Burst_Read_Index(module)                     'get current mem index, number of samples is double of it! 
  'Par_39 = mem_idx 'debug 
