...
'<Header End>
'Process writes 4MHz ADC input 1 values each event (100Hz) to memory of ADwin. no averaging.
...
#Define max_val 4*8*30000*4 '1e5                                'must be devisible by 32 (=4*8) no. of values
...
LowInit:
  ...
  P2_Set_Average_Filter(pattern, 0)                        'no averages
  P2_Burst_Init(module, 1, 0, max_val, 25, 010b)           '250ns per sample, 4MHz
