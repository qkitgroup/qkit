...
'<Header End>
'Process writes 2kHz ADC input 1 values each event (1kHz) to memory of ADwin. Each value is an average of 32 measurements.  
'One buffer cycle takes 0.8 seconds
...
#Define max_val 4*8*50 '1e5                                'must be devisible by 32 (=4*8) no. of values
...
LowInit:
  ...
  P2_Burst_Init(module, 1, 0, max_val, 50000, 010b)        '500 000ns per sample, 2 kHz
