...
'<Header End>
'Process writes 1MHz ADC input 1 values each event (100Hz) to memory of ADwin. Each value is an average of 2 measurements.  
...
#Define max_val 4*8*30000 '1e5                                'must be devisible by 32 (=4*8) no. of values
...
LowInit:
  ...
  P2_Set_Average_Filter(pattern, 1)                        '2**1 averages
  P2_Burst_Init(module, 1, 0, max_val, 100, 010b)         '1000ns per sample, 1MHz
