...
class IV_curve(object):
    ...
    def get_Ic_dft(self, I=None, V=None, s=10, Ir=False, tol_offset=20e-6, window=5, peak_finder=sig.find_peaks, **kwargs):
        """
    ...
    def _classify_jump(self, I, V, Y, peaks, tol_offset=20e-6, window=5, Ir=False):
        ...
        if self.scan_dim == 1:
            ...
            #                          map(lambda (p, d): dict(p.items() + d.items()), zip(zip(*Ps)[1], Ds)),
            #                          zip(np.transpose(peaks, (1, 0, 2)),
