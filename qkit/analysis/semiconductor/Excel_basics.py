...
class PlotterBiascoolingMinimalTopgate(PlotterSemiconInit):
    ...
    def plot(self, data, savename="min_TG_biascooling", shape="^", size=100, transparency=1):
        ...
        plt.grid()
        red_patch = mpatches.Patch(color='red', label='TG left SET')
        blue_patch = mpatches.Patch(color='blue', label='TG right SET')
        plt.legend(handles=[ red_patch, blue_patch])
        plt.savefig(f"{savename}.png", dpi=self.set_dpi, bbox_inches=self.set_bbox_inches)
        plt.show() 
        self.close_delete()
...
class PlotterBiascoolingDifferenceTopgateGates(PlotterSemiconInit):
    ...
    def plot(self, data, savename="Dif_TG_other_gates_biascooling", shape="^", size=100, transparency=1):
        self.ax.set_title("Difference between TG and other gates depending on Bias Cooling")
        self.ax.set_xlabel("Bias Cooling Voltage (V)")
        ...
        plt.grid()
        red_patch = mpatches.Patch(color='red', label='(TG - gates) left SET')
        blue_patch = mpatches.Patch(color='blue', label='(TG - gates) right SET')
        plt.legend(handles=[ red_patch, blue_patch])
        plt.savefig(f"{savename}.png", dpi=self.set_dpi, bbox_inches=self.set_bbox_inches)
        plt.show() 
        self.close_delete()
...
class PlotterBiascoolingDifferenceBarrierGates(PlotterSemiconInit):
    ...
    def plot(self, data, savename="Dif_barrier_gates_biascooling", shape="^", size=100, transparency=1):
