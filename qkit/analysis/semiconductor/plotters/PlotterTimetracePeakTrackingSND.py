...
class PlotterTimetracePeakTrackingSND(SemiFigure):
    ...
    def plot(self, settings:dict, data:dict, data_peak_tracking):
        ...
        self.ax.plot(data_peak_tracking["freq"], np.sqrt(data_peak_tracking["spectrogram"]), "or", markersize=self.dotsize, label="peak tracking", markerscale=3)
        self.ax.plot(data["freq"], np.sqrt(data["spectrogram"] / fit_params_plunger['fit_coef'][0]), "ok", markersize=self.dotsize, label="timetrace", markerscale=3)
