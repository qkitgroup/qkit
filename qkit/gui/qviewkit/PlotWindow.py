...
from qkit.gui.qviewkit.PlotWindow_lib import (
    _display_1D_data,
    _display_1D_view,
    _display_2D_data,
    _display_table,
...
class PlotWindow(QWidget, Ui_Form):
    ...
    def update_plots(self):
        ...
        try:
            if self.ds.attrs.get("xy_0", ""):
                x_ds_name_view = _get_name(
                    _get_ds(
                        self.ds,
                        _get_ds(
                            self.ds, str3(self.ds.attrs.get("xy_0", "")).split(":")[1]
                        ).attrs.get("x_ds_url", ""),
                    )
                )
                y_ds_name_view = _get_name(
                    _get_ds(
                        self.ds,
                        _get_ds(
                            self.ds, str3(self.ds.attrs.get("xy_0", "")).split(":")[1]
        ...
        if self._windowJustCreated:
            ...
            window_title = (
                str(self.dataset_url.split("/")[-1]) + " " + str(self.DATA.filename)
            )
        ...
        try:
            if self.view_type == view_types["1D-V"]:
                ...
                if not self.graphicsView or self._onPlotTypeChanged:
                    self._onPlotTypeChanged = False
                    self.graphicsView = ImageViewMplColorMaps(
                        self.obj_parent, view=pg.PlotItem()
                    )
                    self.graphicsView.setObjectName(self.dataset_url)
                    self.addQvkMenu()
            ...
            print(
                "PlotWindow: Value Error; Dataset not yet available", self.dataset_url
            )
            print(e)
        except OSError as e:
            print(
                "PlotWindow: OS Error; Probably a read/write conflict occured",
                self.dataset_url,
            )
            print(e)
    ...
    def _setup_signal_slots(self):
        ...
        if self.ds_type == ds_types["vector"] or self.ds_type == ds_types["coordinate"]:
            self.PlotTypeSelector.currentIndexChanged.connect(
                self._onPlotTypeChangeVector
            )

        elif self.ds_type == ds_types["matrix"] or self.ds_type == -1:
            self.PlotTypeSelector.currentIndexChanged.connect(
                self._onPlotTypeChangeMatrix
            )
    ...
    def addQvkMenu(self, menu=None):
        ...
        self.linestyle_selector.pointLine = QAction(
            "Point+Line", self.qvkMenu, checkable=True
        )
        ...
        linear_correction = QAction(
            "linearly correct data", self.qvkMenu, checkable=True
        )
        ...
        self.manipulation_menu = {
            ...
            "sub_offset_avg_y": offset_correction,
            "norm_data_avg_x": norm_correction,
            "histogram": histogram,
        }
        ...
        note = QAction(
            "Manipulations are executed in the order shown here.",
            self.qvkMenu,
            enabled=False,
            font=notefont,
        )
        ...
        self.qvkMenu.setWindowTitle(
            "Qviewkit "
            + str(self.dataset_url.split("/")[-1])
            + " "
            + str(self.DATA.filename)
        )
    ...
    def updateManipulationMenu(self):
        for m in self.manipulation_menu:
            self.manipulation_menu[m].setChecked(
                bool(self.manipulation & self.manipulations[m])
            )
...
class ImageViewMplColorMaps(pg.ImageView):
    ...
    def __init__(self, parent=None, name="ImageView", view=None, imageItem=None, *args):
        super(ImageViewMplColorMaps, self).__init__(
            parent=parent, name=name, view=view, imageItem=imageItem, *args
        )
