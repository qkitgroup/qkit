...
def _display_view(self, graphicsView):
    """
    WORK IN PROGRESS (YS)
    displays the 1d & 2d plot(s) views from the respective datasets.
    ...
    for i in range(overlay_num + 1):
        ...
        ds_urls = [xyurls.split(":")[0], xyurls.split(":")[1]]
        if xyurls:
            z_url = self.ds.attrs.get("z_" + str(i), "")
            err_url = self.ds.attrs.get("xy_" + str(i) + "_error", "")
            if z_url:
                ds_urls.append(z_url)
            elif err_url: # If there is a z_url, errors aren't accepted since impossible in a colorplot
                ds_urls.append(err_url)
            dss, names, units, scales = _get_all_ds_names_units_scales(self.ds, ds_urls)
            if dss[0] is None or dss[1] is None:
                print("Could not load view xy_%i: "%i + str(xyurls)+" : Datasets not found.")
            ...
            if z_url: # This block manages with colorplots
                if i > 0:
                    print("Colorplot should be base layer (xy_0), otherwise it covers the other layers.")
                    print("This layer is ignored.")
                    continue

                if dss[2] is None:
                    print("Could not load view z_%i: " % i + str(xyurls) + " : Datasets not found.")
                    continue
                x_ds_type = dss[0].attrs.get('ds_type', ds_types['coordinate'])
                y_ds_type = dss[1].attrs.get('ds_type', ds_types['coordinate'])
                if x_ds_type != ds_types['coordinate'] and x_ds_type != ds_types['vector']:
                    print("x and y datasets must be coordinate or vector for colorplot in view.")
                    continue
                if y_ds_type != ds_types['coordinate'] and y_ds_type != ds_types['vector']:
                    print("x and y datasets must be coordinate or vector for colorplot in view.")
                    continue
                z_ds_type = dss[2].attrs.get('ds_type')
                if z_ds_type != ds_types['matrix']:
                    print("Z-data must be of matrix type for colorplot view. Box not yet supported.")
                    # Fixme: Implement 2d plot for box as well.
                    continue
                try:
                    data = dss[2][()]
                except IOError as e:
                ...
                image_height = abs(dss[1][-1] - dss[1][0])  # if x and y-scales are the same
                pixel_sizex = image_width / (dss[0].size - 1)
                pixel_sizey = image_height / (dss[1].size - 1)
                image_width = image_width + pixel_sizex
                image_height = image_height + pixel_sizey
