...
def _display_1D_view(self, graphicsView):
    ...
    for i in range(overlay_num + 1):
        ...
        if xyurls:
            ...
            if x_ds_type == ds_types["coordinate"] or x_ds_type == ds_types["vector"]:
                if (
                    y_ds_type == ds_types["vector"]
                    or y_ds_type == ds_types["coordinate"]
                ):
                    ...
                    if view_params.get("transpose", False):
                        ...
                        if (
                            self.VTraceXSelector.isEnabled()
                        ):  # Hack to see if the window is freshly generated
                            self.VTraceXSelector.setEnabled(False)
                            self.VTraceYSelector.setValue(
                                view_params.get("default_trace", range_max // 2)
                            )
                        self.VTraceYSelector.setRange(-1 * range_max, range_max - 1)
                        self.VTraceYValue.setText(
                            self._getYValueFromTraceNum(dss[1], self.VTraceYNum)
                        )
                        x_data = dss[0][()]
                        ...
                        self.VTraceXValue.setText(
                            self._getXValueFromTraceNum(dss[1], self.VTraceXNum)
                        )
                        self.VTraceYSelector.setEnabled(False)
            ...
            _axis_timestamp_formatting(
                graphicsView, x_data, units[0], names[0], "bottom"
            )
            ...
            if self.plot_style == self.plot_styles["line"]:
                graphicsView.plot(
                    y=y_data, x=x_data, pen=linecolor, name=names[1], connect="finite"
                )
            ...
            if err_url:
                err = pg.ErrorBarItem(
                    x=x_data, y=y_data, height=err_data, beam=0.25 * scales[0][0]
                )
    ...
    def mouseMoved(mpos):
        ...
        if plIt.sceneBoundingRect().contains(mpos):
            ...
            try:
                self.data_coord = "%e\t%e\t%e\t%e" % (
                    xval,
                    yval,
                    self._last_x_pos - xval,
                    xval / (self._last_x_pos - xval),
                )
    ...
    self.proxy = pg.SignalProxy(
        plVi.scene().sigMouseMoved, rateLimit=15, slot=mouseMoved
    )
...
def _display_1D_data(self, graphicsView):
    ...
    Returns:
        ...
        PlotWindow class.
    """
    if self.ds_type == ds_types["vector"] or (
        self.ds_type == -1 and len(self.ds.shape) == 1
    ):  # last expresson is for old hdf-files
        dss, names, units, scales = _get_all_ds_names_units_scales(
            self.ds, ["x_ds_url"]
        )
        ...
        try:
            x_data = dss[0][()][
                : dss[1].shape[-1]
            ]  # x_data gets truncated to y_data shape if necessarry
        except:
    ...
    elif self.ds_type == ds_types["matrix"] or (
        self.ds_type == -1 and len(self.ds.shape) == 2
    ):  # last expresson is for old hdf-files
        """
        ...
        if self.PlotTypeSelector.currentIndex() == 1:  # y_ds on x-axis
            ...
            if self.TraceXValueChanged:
                ...
                (x0, dx) = _get_axis_scale(
                    _get_ds(self.ds, _get_ds_url(self.ds, "x_ds_url"))
                )
            ...
            x_data = dss[0][()][
                : dss[1].shape[-1]
            ]  # x_data gets truncated to y_data shape if neccessary
        ...
        if self.PlotTypeSelector.currentIndex() == 2:  # x_ds on x-axis
            ...
            if self.TraceYValueChanged:
                ...
                # calc trace number from entered value
                (x0, dx) = _get_axis_scale(
                    _get_ds(self.ds, _get_ds_url(self.ds, "y_ds_url"))
                )
        ...
        if self.TraceZValueChanged:
            ...
            """
            # calc trace number from entered value
            (z0, dz) = _get_axis_scale(
                _get_ds(self.ds, _get_ds_url(self.ds, "z_ds_url"))
            )
    ...
    ## Any data manipulation (dB <-> lin scale, etc) is done here
    x_data, y_data, names[0], names[1], units[0], units[1] = _do_data_manipulation(
        x_data,
        y_data,
        names[0],
        names[1],
    ...
    if _axis_timestamp_formatting(
        graphicsView, x_data, units[0], names[0], "bottom"
    ) or _axis_timestamp_formatting(graphicsView, y_data, units[1], names[1], "left"):
        if not self.user_setting_changed:
    ...
    if self.plot_style == self.plot_styles["line"]:
        graphicsView.plot(
            y=y_data, x=x_data, clear=True, pen=(200, 200, 100), connect="finite"
        )
            ...
            if self.distance_measure[0] is True:  # a ROI is open
                ...
                try:
                    ...
                    td.setText(
                        ...
                        % (
                            pg.siFormat(
                                (yval - y0) / (xval - x0) / sx, suffix=units[1]
                            ),
                            Sx,
                            units[0],
                        )
...
def _display_2D_data(self, graphicsView):
    ...
    if self.ds_type == ds_types["box"]:
        ...
        if self.PlotTypeSelector.currentIndex() == 0:  # y_ds on x-axis; z_ds on y-axis
            if self.TraceXValueChanged:
                # calc trace number from entered value
                num = int(
                    (self._traceX_value - self.ds.attrs.get("x0", 0))
                    / (self.ds.attrs.get("dx", 1))
                )
                self.TraceXNum = num
                self.TraceXSelector.setValue(self.TraceXNum)
                self.TraceXValueChanged = False
        ...
        if self.PlotTypeSelector.currentIndex() == 1:  # x_ds on x-axis; z_ds on y-axis
            if self.TraceYValueChanged:
                # calc trace number from entered value
                num = int(
                    (self._traceY_value - self.ds.attrs.get("y0", 0))
                    / (self.ds.attrs.get("dy", 1))
                )
                self.TraceYNum = num
                self.TraceYSelector.setValue(self.TraceYNum)
                self.TraceYValueChanged = False
        ...
        if self.PlotTypeSelector.currentIndex() == 2:  # x_ds on x-axis; y_ds on y-axis
            if self.TraceZValueChanged:
                # calc trace number from entered value
                num = int(
                    (self._traceZ_value - self.ds.attrs.get("z0", 0))
                    / (self.ds.attrs.get("dz", 1))
                )
                self.TraceZNum = num
                self.TraceZSelector.setValue(self.TraceZNum)
                self.TraceZValueChanged = False
    ...
    _, data, _, _, _, units[2] = _do_data_manipulation(
        None,
        data,
        None,
        None,
    ...
    if np.all(np.isnan(data)):
        ...
        print(
            "Your Data array is all NaN. I set the first value to not blow up graphics window."
        )
    graphicsView.setImage(
        data,
        pos=(scales[0][0] - scales[0][1] / 2.0, scales[1][0] - scales[1][1] / 2.0),
    ...
    graphicsView.roi.sigClicked.connect(
        lambda: self.clickRoi(graphicsView.roi.pos(), graphicsView.roi.size())
    )
        ...
        if self.distance_measure[0] is True:  # a ROI is open
            ...
            try:
                ...
                td.setText(
                    "%s/%s%s"
                    % (
                        pg.siFormat((yval - y0) / (xval - x0) / sx, suffix=units[1]),
                        Sx,
                        units[0],
                    )
                )
    ...
    self.proxy = pg.SignalProxy(
        imVi.scene().sigMouseMoved, rateLimit=15, slot=mouseMoved
    )
...
def _get_unit(ds):
    ...
    """
    try:
        return str3(ds.attrs.get("unit", b"_none_"))  # .decode('utf-8')
...
def _get_name(ds):
    ...
    if ds is None:
        return "_none_"
    try:
        return str3(ds.attrs.get("name", b"_none_"))  # .decode("utf-8")
    except AttributeError as e:
        # print(ds)
...
def _do_data_manipulation(
    ...
    if manipulation & manipulations["linear"]:
        if len(y_data.shape) == 1:
            y_data = y_data - np.nan_to_num(
                np.linspace(y_data[0], y_data[-1], len(y_data))
            )
        elif len(y_data.shape) == 2:
            y_data = y_data - np.outer(
                y_data[:, -1] - y_data[:, 0], np.linspace(0, 1, y_data.shape[1])
            )
        else:
        ...
        if colorplot:
            ...
            if manipulation & manipulations["remove_zeros"]:
                y_data[
                    ...
                    np.NaN
                )  # replace all exact zeros in the hd5 y_data with NaNs, otherwise the 0s in uncompleted files blow up the colorscale
    ...
    if manipulation & manipulations["histogram"]:
        if y_data.ndim == 1:
            ...
            x_data = x_data[:-1] + (x_data[1] - x_data[0]) / 2.0
            x_name = y_name
            x_unit = y_unit
            y_name = "counts"
            y_unit = ""
...
def _axis_timestamp_formatting(graphicsView, data, unit, name, orientation):
    if (
        unit == "s" and data[tuple(-1 for x in data.shape)] > 9.4e8
    ):  # about the year 2000 in unixtime
