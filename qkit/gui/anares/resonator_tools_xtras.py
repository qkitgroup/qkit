# Todo!!!! error for Qi_no_corr not yet programmed
...
def cut_data(f_data,z_data,f1,f2):
    ...
    f_data = [f_data[i] for i in range(pos1,pos2,1)]
    z_data = [z_data[i] for i in range(pos1,pos2,1)]
    return f_data, z_data
...
def get_delay(f_data,z_data,delay=None,ignoreslope=True,guess=True):
    ...
    if ignoreslope==True:
        ...
        z_data = [(np.absolute(z_data[i])-A2*(f_data[i]-fr)) * np.exp(np.complex(0,np.angle(z_data[i]))) for i in range(len(f_data))] #usually not necessary
    if delay==None:
        if guess==True:
            delay = rt.guess_delay(f_data,z_data)
        else:
            delay=0.
        delay = rt.fit_delay(f_data,z_data,delay,maxiter=100)
...
def do_calibration(f_data,z_data,ignoreslope=True,guessdelay=True):
    ...
    z_data = [(z_data[i]-params[1]*(f_data[i]-params[4]))*np.exp(np.complex(0,2.*np.pi*delay*f_data[i])) for i in range(len(f_data))]
...
def do_normalization(f_data,z_data,delay,amp_norm,alpha,A2,frcal):
    ...
    '''
    return [(z_data[i]-A2*(f_data[i]-frcal))/amp_norm*np.exp(np.complex(0,-alpha+2.*np.pi*delay*f_data[i])) for i in range(len(f_data))]
...
def circlefit(f_data,z_data,fr=None,Qr=None,refine_results=False,calc_errors=True):
    ...
    z_data_corr = rt.center(z_data,complex(xc,yc))
    theta0, Qr, fr = rt.phase_fit(f_data,z_data_corr,theta0,Qr,fr)
    #print "Qr from phasefit is: " + str(Qr)
    absQc = Qr/(2.*r0)    #*(np.absolute(np.complex(xc,yc))+r0)
    complQc = absQc*np.exp(np.complex(0,(-1.)*phi0))
    ...
    #chi_square = (np.array([np.absolute(z_data[i] - (1 - Qr/absQc*np.exp(np.complex(0,phi0))/np.complex(1,2*Qr*(f_data[i]/fr-1)))  )  for i in range(len(f_data))])**2).sum() / float(len(f_data))
    ...
    if calc_errors==True:
        ...
        if cov!=None:
            ...
            ##Qi_no_corr_err = np.sqrt( np.sqrt((dQr*cov[2][2])**2 + (dabsQc*cov[1][1])**2+(2*dQr*dabsQc*cov[2][1])) ) #with correlations
            Qi_no_corr_err = np.sqrt((dQr**2*cov[2][2]) + (dabsQc**2*cov[1][1])+(2*dQr*dabsQc*cov[2][1]))  #with correlations
            #Qi_no_corr_err = np.sqrt( np.sqrt((dQr*cov[2][2])**2 + (dabsQc*cov[1][1])**2) ) #without correlations
            ...
            Qi_dia_corr_err =  np.sqrt(err1+2*err2)  #with correlations
            #Qi_dia_corr_err = np.sqrt( np.sqrt(err1) )  #without correlations
        ...
        fun2 = lambda x: np.array([rt.residuals_notch_ideal(x,f_data[i],z_data[i])**2 for i in range(len(f_data))])
        chi_square = chisqr = 1/float(len(f_data)-len(p)) * (fun2(p)).sum()
...
def fit_S21data(f_data,z_data,amp_norm,alpha,delay,Qr,absQc,phi0,fr,maxiter=0):
    ...
    ##p_final[0] = fr,absQc,Qr,phi0,delay,a,alpha
    ...
    #!!!! error for Qi_no_corr not yet programmed
