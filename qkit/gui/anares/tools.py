...
def loadspecdata2(filename,var_col=0,f_col=1,y1_col=2,y2_col=3,sformat='Real/Imag'):
    ...
    return np.array(i_data),np.array(f_data),np.array(z_data)
...
def S21(f,fr=10e9,Qr=900.0,Qc=1000.,phi=0.,a=1.,alpha=0.,delay=.0):
    return a*np.exp(1j*alpha)*np.exp(-1j*2.*np.pi*f*delay)*(1.-Qr/Qc*np.exp(1j*phi)/(1.+1j*2.*Qr*(f-fr)/fr))
...
def skewed_lorentz_function(x,A1,A2,A3,A4,fr,Qr):
    ...
    #else:
        #return 0.0
    return np.sqrt(A1+A2*(x-fr)+(A3+A4*(x-fr))/(1.+4.*Qr**2*((x-fr)/fr)**2))
