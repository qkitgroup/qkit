...
import qkit
from qkit.gui.qviewkit.PlotWindow_lib import str3
...
class h5plot(object):
    ...
    def plt(self):
        ...
        save_name = (
            str(os.path.basename(self.filedir))[0:6]
            + "_"
            + self.key.replace("/entry/", "").replace("/", "_")
        )
        if self.comment:
    ...
    def plt_vector(self):
        ...
        """
        self.ax.set_title(
            self.hf._filename[:-3] + " " + str3(self.ds.attrs.get("name", "_name_"))
        )
        ...
        self.y_label = (
            ...
            + " ("
            + self._unit_prefixes[self.y_exp]
            + self.ds.attrs.get("unit", "_unit_").decode()
            + ")"
        )
        ...
        try:
            ...
            self.x_label = (
                self.x_data.attrs.get("name", "_xname_").decode()
                + " ("
                + self._unit_prefixes[self.x_exp]
                + self.x_data.attrs.get("unit", "_xunit_").decode()
                + ")"
            )
        except Exception:
        ...
        plot_style = self.plot_styles[
            int(not (len(self.y_data) - 1)) * 2
        ]  # default is 'x' for one point and '-' for lines
        ...
        try:
            self.ax.plot(
                np.array(self.x_data)[0 : len(self.y_data)] * 10 ** int(-self.x_exp),
                self.y_data[0 : len(self.x_data)] * 10 ** int(-self.y_exp),
                plot_style,
            )  # JF: If measurement is stopped by watchdog, plots are still saved due to cropping of x-axis.
        except TypeError:
    ...
    def plt_matrix(self):
        ...
        self.x_label = (
            self.x_ds.attrs.get("name", "_xname_").decode()
            + " ("
            + self._unit_prefixes[self.x_exp]
            + self.x_ds.attrs.get("unit", "_xunit_").decode()
            + ")"
        )
        self.y_ds = self.hf[self.y_ds_url]
        self.y_exp = self._get_exp(np.array(self.y_ds))
        self.y_label = (
            self.y_ds.attrs.get("name", "_yname_").decode()
            + " ("
            + self._unit_prefixes[self.y_exp]
            + self.y_ds.attrs.get("unit", "_yunit_").decode()
            + ")"
        )
        self.ds_data = np.array(self.ds).T  # transpose matrix to get x/y axis correct
        self.ds_exp = self._get_exp(self.ds_data)
        self.ds_data *= 10.0 ** -self.ds_exp
        self.ds_label = (
            self.ds.attrs.get("name", "_name_").decode()
            + " ("
            + self._unit_prefixes[self.ds_exp]
            + self.ds.attrs.get("unit", "_unit_").decode()
            + ")"
        )

        x_data = np.array(self.x_ds) * 10.0 ** -self.x_exp
        x_min, x_max = np.amin(x_data), np.amax(x_data)
        dx = self.x_ds.attrs.get("dx", (x_data[-1] - x_data[0]) / (len(x_data) - 1))
        y_data = np.array(self.y_ds) * 10.0 ** -self.y_exp
        ...
        # plot
        self.ax.set_title(
            self.hf._filename[:-3] + " " + self.ds.attrs.get("name", "_name_").decode()
        )
    ...
    def plt_box(self):
        ...
        self.z_label = (
            self.z_ds.attrs.get("name", "_zname_").decode()
            + " ("
            + self._unit_prefixes[self.z_exp]
            + self.z_ds.attrs.get("unit", "_zunit_").decode()
            + ")"
        )
        self.ds_data = np.array(self.ds)[
            :, :, self.ds.shape[2] // 2
        ].T  # transpose matrix to get x/y axis correct
        self.ds_exp = self._get_exp(self.ds_data)
        self.ds_data *= 10.0 ** -self.ds_exp
        self.ds_label = (
            self.ds.attrs.get("name", "_name_").decode()
    ...
    def plt_view(self):
        ...
        for i, x_ds in enumerate(self.ds_xs):
            ...
            if (
                x_ds.attrs.get("ds_type", 0) == ds_types["coordinate"]
                or x_ds.attrs.get("ds_type", 0) == ds_types["vector"]
            ):
                if (
                    y_ds.attrs.get("ds_type", 0) == ds_types["vector"]
                    or y_ds.attrs.get("ds_type", 0) == ds_types["coordinate"]
                ):
            ...
            plot_style = self.plot_styles[
                view_params.get("plot_style", int(not (len(y_data) - 1)) * 2)
            ]  # default is 'x' for one point and '-' for lines
            if err_ds:
                self.ax.errorbar(
                    x_data,
                ...
                try:
                    x_exp = self._get_exp(
                        x_data
                    )  # caculate order of magnitude for unit-prefix
            ...
            self.x_label += (
                " (" + self._unit_prefixes[x_exp] + self.x_unit.decode() + ")"
            )
            self.y_label += (
                " (" + self._unit_prefixes[y_exp] + self.y_unit.decode() + ")"
            )
        self.ax.legend()
    ...
    def _get_vrange(self, data, percent):
        ...
        _min -= (_max - _min) * percent / (100.0 - 2 * percent)
        _max += (_max - _min) * percent / (100.0 - 2 * percent)
        return [_min, _max]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--file", required=True, type=str, help="hdf/h5 filename to open"
    )
    parser.add_argument(
        "-c", "--comment", type=str, help="(optional) comment to append at filenames"
    )
    parser.add_argument(
        "-pdf",
        "--save-pdf",
        default=False,
        action="store_true",
        help="(optional) save default plots",
    )
